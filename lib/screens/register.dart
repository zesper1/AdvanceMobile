...
class _RegisterScreenState extends ConsumerState<RegisterScreen> {
  ...
  // MODIFIED: Replaced course and year level controllers with simple string variables
  ...
  void dispose() {
    ...
    // No need to dispose course/year controllers anymore
    super.dispose();
  }

  // NEW: Helper function to convert dropdown string to integer for the database
  int _parseYearLevel(String year) {
    switch (year) {
      ...
      case '3RD': return 3;
      case '4TH': return 4;
      case 'SHS': return 0; // Using 0 for SHS, or you can use 11/12
      default: return 0;
    }
  }
  ...
  Future<void> _signUp() async {
    ...
    if (!_agreedToTerms) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('You must agree to the terms and conditions to register.'), backgroundColor: Colors.orange),
      );
      return;
    }
  ...
  Widget _buildRegisterForm(BuildContext context, bool isLoading) {
    return Column(
      ...
      children: [
        const Text('Sign Up', style: TextStyle(fontSize: 24, fontWeight: FontWeight.bold), textAlign: TextAlign.center),
        const SizedBox(height: 24),
        
        // MODIFIED: Using the new reusable dropdown
        ...
        if (_selectedRole != null) ...[
          ...
          if (_selectedRole == 'Student') ...[
            ...
            TextFormField(
              ...
              decoration: const InputDecoration(hintText: 'Student ID Number', prefixIcon: Icon(Icons.badge_outlined)),
              validator: (value) => value == null || value.isEmpty ? 'Please enter your Student ID' : null,
            ),
            const SizedBox(height: 16),
            // MODIFIED: Using the new reusable dropdown for Course
            CustomDropdownFormField(
             ...
             const SizedBox(height: 16),
             // MODIFIED: Using the new reusable dropdown for Year Level
            CustomDropdownFormField(
              ...
              validator: (value) => value == null ? 'Please select a year level' : null,
            ),
          ],
        ...
        const SizedBox(height: 16),
        _buildAuthSwitch(context: context, label: 'Already have an account?', buttonText: 'Login', onTap: () { Navigator.pop(context); }),
      ],
    );
  }
...
// NEW: A reusable custom dropdown widget to keep our form code clean (DRY principle)
